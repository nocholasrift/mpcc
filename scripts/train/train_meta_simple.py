...
from meta_train_realworld import TrainManager, CustomEnv
from oyster.rlkit.configs.default import default_config
...
def collect_data(trainer, num_samples, idx, args):

    # start from the prior
    trainer.agent.clear_z()

    while num_samples > 0:
        rospy.set_param("/train/num_samples", num_samples)
        rospy.set_param("/train/max_path_length", default_config["max_path_length"])

        # run simulation
        run_sim(args, idx)
        num_samples -= trainer.encoder_buffer_size(idx)
...
if __name__ == "__main__":
    ...
    for it in tqdm(range(num_updates)):
        trainer.start_epoch(it)
        trainer.training_mode(True)

        if it == 0:
            print("collecting initial pool of data")

        for i in range(default_config["num_tasks_sample"]):
            idx = np.random.randint(default_config["n_train_tasks"])
            trainer.clear_encoder_buffer(idx)
            rospy.set_param("/train/task_id", idx)

            # collect some data with z ~ prior
            collect_data(trainer, default_config["num_steps_prior"], idx, args)

            # collect some data with z ~ posterior
            # typically, this is 0 during training
            collect_data(trainer, default_config["num_steps_posterior"], idx, args)

            # even if encoder is trained on prior only, policy needs to learn to handle
            # z ~ posterior
            collect_data(
                trainer, default_config["num_extra_rl_steps_posterior"], idx, args
            )

        # Sample training tasks and compute gradient updates on parameters
        trainer.train_sac()

        # not sure if this needs to be done every loop but just in case...
